...
HEART_ENDPOINT = "heart.ping.pong";
...
class ThriftBaseClient(object):
    ...
    def test_connection(self):
        ...
        try:
            ...
            self.dispatcher(NetUtils.getIp(),'.'.join(endpoints[0:-2]),endpoints[-2],endpoints[-1],list())
            return True
        except:
            return False
...
class BaseClientPool(object):
    ...
    def __init__(self, service, timeout=30, name=None, raise_empty=False,
                 max_conn=30, min_conn=2, connection_class=ThriftClient, keepalive=None,
    ...
    def fill_connection_pool(self):
        ...
        """
        rest_size = self.min_conn - self.pool_size()
...
class MultiServerClientBase(BaseClientPool):
    def __init__(self, service, servers, timeout=30, name=None,
                 raise_empty=False, max_conn=30, min_conn=2, connection_class=ThriftClient,
                 keepalive=None, tracking=False, tracker_factory=None,
                 check_interval=360, use_limit=None):
    ...
    def maintain_connections(self):
        sleep_time = max(10, self.timeout-5)
        ...
        while True:
            ...
            for _ in range(pool_size):
                ...
                if (time.time()-conn.latest_use_time < self.check_interval or
                        conn.test_connection()) and self.pool_size()<=self.min_conn:
                    self.put_back_connection(conn)
                else:
                    conn.close()


class MultiClientPool(MultiServerClientBase):
    def __init__(self, *args, **kwds):
        super(MultiClientPool, self).__init__(*args, **kwds)
